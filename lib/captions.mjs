...
// Detecta faixas pretas (vídeo horizontal centrado em canvas vertical).
export async function detectFormat(videoPath, probe) {
  ...
  if (!crops.length) return { type: 'vertical', top: 0, bottom: probe.height };
  ...
  if (h < probe.height * 0.8 && y > probe.height * 0.05) {
    return { type: 'horizontal', top: y, bottom: y + h };
  }
  return { type: 'vertical', top: 0, bottom: probe.height };
}
...
export async function burnOverlays({ videoPath, events, hook, probe, format, pngDir, outPath, workDir }) {
  const stripH = Math.round(probe.width * 0.5); // faixa generosa; texto centrado nela
  const H = probe.height;

  let capY, hookY, hookEnable;
  if (format.type === 'horizontal') {
