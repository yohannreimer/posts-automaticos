...
// Transcreve só a abertura de um vídeo (gancho falado) — rápido, pra
// enriquecer posts antigos sem legenda na aba de análise.
export async function transcribeHook(videoUrl, workDir) {
  await mkdir(workDir, { recursive: true });
  const video = path.join(workDir, 'hook-src.mp4');
  const wav = path.join(workDir, 'hook.wav');
  const jsonPrefix = path.join(workDir, 'hook-words');
  ...
  await run('ffmpeg', ['-y', '-v', 'error', '-t', '25', '-i', video, '-ar', '16000', '-ac', '1', '-c:a', 'pcm_s16le', wav]);
  await run(WHISPER_BIN, ['-m', WHISPER_MODEL, '-f', wav, '-l', 'pt', '-oj', '-of', jsonPrefix]);

  const data = JSON.parse(await readFile(`${jsonPrefix}.json`, 'utf8'));
  const text = (data.transcription || []).map((s) => s.text).join(' ').replace(/\s+/g, ' ').trim();
