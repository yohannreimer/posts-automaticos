...
// Vídeos vão direto pro litterbox (imgbb é só imagem). Limite: 1GB.
export async function uploadPublicVideo(buffer, name = 'reel') {
  const form = new FormData();
  form.set('reqtype', 'fileupload');
  form.set('time', '1h');
  form.set('fileToUpload', new Blob([buffer], { type: 'video/mp4' }), `${name}.mp4`);
  const res = await fetch('https://litterbox.catbox.moe/resources/internals/api.php', {
    method: 'POST',
    body: form,
  });
  const text = (await res.text()).trim();
  if (!res.ok || !text.startsWith('https://')) {
    throw new Error(`Falha no upload temporário do vídeo (litterbox ${res.status}): ${text.slice(0, 200)}`);
  }
  return text;
}
