...
// videoUrl: URL pública do MP4. Retorna { id, permalink }.
// O processamento de vídeo no Instagram é mais lento que imagem (30-120s).
export async function publishReelToInstagram({ videoUrl, caption = '' }) {
  if (!instagramEnabled()) {
    throw new Error(
  ...
  await waitForContainer(container.id, { timeoutMs: 300000 });
  const published = await ig(`${userId}/media_publish`, { creation_id: container.id });
  let permalink = '';
  try {
